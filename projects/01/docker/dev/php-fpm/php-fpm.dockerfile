...
RUN --mount=type=cache,target=/var/cache/apk \
    set -ex \
    && apk add libpq icu libpng libjpeg-turbo freetype libzip shadow gettext \
...
COPY ./common/php/www.conf.template /var/tmp/php-fpm.d/www.conf.template
RUN addgroup $user \
    && adduser -DSH -u $uid -G $user $user \
    && adduser www-data $user \
    && sh -c envsubst ${app_dir} ${user} < /var/tmp/php-fpm.d/www.conf.template > /usr/local/etc/php-fpm.d/www.conf \
